...
// SEO Metadata for Root Canal Page
export async function generateMetadata({ params }) {
    const { locale } = await params;
    const t = await getTranslations('rootCanal.metadata');

    const baseUrl = 'https://toothmatedental.com';
    const canonicalPath = locale === 'ar' ? '/حشو-العصب/خدماتنا' : '/our-services/root-canal';
    ...
    return {
        ...
        openGraph: {
            ...
            images: [
                {
                    url: `${baseUrl}/images/root-canal.webp`,
                    width: 1200,
                    height: 630,
                    alt: locale === 'ar' ? 'علاج قناة الجذر في عيادة توث ميت' : 'Root Canal Treatment at Tooth Mate Clinic'
                },
            ],
        },
        twitter: {
            ...
            title: t('ogTitle'),
            description: t('ogDescription'),
            images: [`${baseUrl}/images/root-canal.webp`],
        },
        robots: {
            index: true,
            follow: true,
...
export default async function RootCanalService({ params }) {
    const { locale } = await params;
    const t = await getTranslations('rootCanal');

    const baseUrl = 'https://toothmatedental.com';
    const canonicalPath = locale === 'ar' ? '/حشو-العصب/خدماتنا' : '/our-services/root-canal';

    // Structured Data for Root Canal Service
    const rootCanalStructuredData = {
    ...
    const faqStructuredData = {
        ...
        "mainEntity": [
            {
                "@type": "Question",
                "name": locale === 'ar' ? "هل علاج قناة الجذر مؤلم؟" : "Is root canal treatment painful?",
                "acceptedAnswer": {
                    "@type": "Answer",
                    "text": locale === 'ar' ? "لا، العلاج يتم تحت التخدير الموضعي مما يجعله مريحاً وخالياً من الألم" : "No, the treatment is performed under local anesthesia making it comfortable and pain-free"
                }
            },
            {
                "@type": "Question",
                "name": locale === 'ar' ? "كم تدوم نتيجة علاج قناة الجذر؟" : "How long do root canal results last?",
                "acceptedAnswer": {
                    "@type": "Answer",
                    "text": locale === 'ar' ? "مع العناية المناسبة، يمكن أن تدوم النتائج مدى الحياة" : "With proper care, results can last a lifetime"
                }
            },
            {
                "@type": "Question",
                "name": locale === 'ar' ? "ما هي البدائل لعلاج قناة الجذر؟" : "What are the alternatives to root canal treatment?",
                "acceptedAnswer": {
                    "@type": "Answer",
                    "text": locale === 'ar' ? "البديل الوحيد هو خلع السن، لكننا نوصي بالحفاظ على السن الطبيعي whenever possible" : "The only alternative is tooth extraction, but we recommend preserving natural teeth whenever possible"
                }
            }
        ]
    };
    ...
    return (
        <section className='bg-[#D3F3FF]'>
            ...
            <script
                ...
                dangerouslySetInnerHTML={{ __html: JSON.stringify(rootCanalStructuredData) }}
                key="root-canal-service-schema"
            />
            <script
                type="application/ld+json"
            ...
            {/* Hero Section */}
            <div
                className="relative bg-[url('/images/root-canal.webp')] bg-cover bg-center h-[400px] w-full flex justify-center items-center text-white font-bold"
                role="banner"
            ...
            <section className="container-custom" itemScope itemProp="mainContentOfPage">
                ...
                {/* Main Service Image */}
                <div className="w-full h-[300px] md:h-[650px] relative rounded-4xl overflow-hidden my-8">
                    <Image
                        src="/images/root-canal-filling.webp"
                ...
                {/* When You Need Root Canal Section */}
                <div className="my-12">
                    <h2 className="text-2xl md:text-3xl font-bold mb-6">
                        {t('whenNeedRootCanal.title')}
                    </h2>
                    <ul className="space-y-6">
                        {[1, 2, 3, 4, 5].map((item) => (
                            <li key={item} className="flex items-start gap-4">
                                ...
                                <p className="text-[18px] md:text-2xl leading-relaxed">
                                    {t(`whenNeedRootCanal.symptom${item}`)}
                                </p>
                            </li>
                        ))}
                    </ul>
                ...
                {/* Time and Results Section */}
                <div className="my-12">
                    <h2 className="text-2xl md:text-3xl font-bold mb-6">
                        {t('timeAndResults.title')}
                    </h2>
                    <div className="grid md:grid-cols-2 gap-8">
                        ...
                        <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-200">
                            <div className="flex items-center gap-4 mb-4">
                                <Image
                                    src="/icons/therapy.png"
                                    width={50}
                                    height={50}
                                    alt={t('timeAndResults.timeIconAlt')}
                                    itemProp="image"
                                />
                                <h3 className="text-xl font-bold text-[#0178A3]">
                                    {t('timeAndResults.timeTitle')}
                                </h3>
                            </div>
                            <p className="text-gray-700 mb-4">
                                {t('timeAndResults.timeDescription')}
                            </p>
                            <div className="space-y-2">
                                {[1, 2, 3].map((item) => (
                                    <div key={item} className="flex items-center gap-2">
                                        <div className="w-2 h-2 bg-[#0178A3] rounded-full"></div>
                                        <span className="text-sm">{t(`timeAndResults.timePoint${item}`)}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-200">
                            <div className="flex items-center gap-4 mb-4">
                                <Image
                                    ...
                                    width={50}
                                    height={50}
                                    alt={t('timeAndResults.resultsIconAlt')}
                                    itemProp="image"
                                />
                                <h3 className="text-xl font-bold text-[#0178A3]">
                                    {t('timeAndResults.resultsTitle')}
                                </h3>
                            </div>
                            <p className="text-gray-700 mb-4">
                                {t('timeAndResults.resultsDescription')}
                            </p>
                            <div className="space-y-2">
                                {[1, 2, 3].map((item) => (
                                    <div key={item} className="flex items-center gap-2">
                                        <div className="w-2 h-2 bg-[#0178A3] rounded-full"></div>
                                        <span className="text-sm">{t(`timeAndResults.resultsPoint${item}`)}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                ...
                <div className="flex flex-col lg:flex-row justify-between items-center md:container-custom my-10">
                    ...
                    <div className={`flex justify-center items-center gap-3 order-1 lg:order-2 ${locale === 'en' ? 'lg:mr-9' : 'lg:ml-9'}`}>
                        ...
                        />
                        <Image
                            src="/images/root-canal-photo.webp"
                            width={186}
                            height={180}
