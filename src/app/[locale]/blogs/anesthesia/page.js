...
export async function generateMetadata({ params }) {
    ...
    const t = await getTranslations({ locale, namespace: 'blog-anesthesia' });

    const baseUrl = 'https://toothmatedental.com';
    const articlePath = locale === 'ar' ? '/نصائح-يومية-للحفاظ-علي-صحة-أسنانك/مقالاتنا' : '/our-blogs/daily-advices-for-healthy-teeth';

    return {
        title: t('title'),
        description: t('intro'),
        keywords: "نصائح العناية بالأسنان, صحة الفم, تنظيف الأسنان, خيط الأسنان, العناية باللثة, وقاية تسوس الأسنان, نظافة الفم, فرشاة الأسنان, غسول الفم, زيارة طبيب الأسنان, dental care tips, oral health, teeth cleaning, dental floss, gum care, cavity prevention, oral hygiene, toothbrush, mouthwash, dentist visit",
        alternates: {
        ...
        openGraph: {
            ...
            tags: ["العناية بالأسنان", "صحة الفم", "نصائح طبية"]
        },
        twitter: {
            card: 'summary_large_image',
            title: t('title'),
            description: t('intro'),
            images: [`${baseUrl}/general-anesthesia.jpeg`],
        },
        robots: {
            index: true,
            follow: true,
...
export default async function AdvicesBlog({ params }) {
    const { locale } = await params;
    const t = await getTranslations({ locale, namespace: 'blog-anesthesia' });
    ...
    const baseUrl = 'https://toothmatedental.com';
    const articlePath = locale === 'ar' ? '/كل-ما-تحتاج-معرفتة-عن-التخدير-الكلي-للاسنان/مقالاتنا' : '/our-blogs/all-you-need-to-know-about-general-anesthesia';
    ...
    const structuredData = {
        ...
        "headline": t('title'),
        "description": t('intro'),
        "image": `${baseUrl}/images/blog-teeth.webp`,
        ...
        "keywords": "نصائح العناية بالأسنان, صحة الفم, تنظيف الأسنان, خيط الأسنان, العناية باللثة",
        "articleSection": "صحة الأسنان",
    ...
    return (
        <section className='bg-[#D3F3FF]'>
            ...
            <section
                className="relative
                    bg-[url('/images/general-anesthesia.jpeg')] 
                    bg-cover bg-center 
                    h-[400px] w-full 
                    flex justify-center 
                    items-center
            ...
            <article className='container-custom max-w-6xl mx-auto ' itemScope itemType="https://schema.org/Article">
                ...
                <p className='text-center text-[13px] md:text-[16px] font-semibold my-8 text-[#0178A3]'>
                    {t('readingTime')}
                </p>
                ...
                {/* CTA Section - Added similar to implants blog */}
                {ctaData && (
                        ...
                        <div className='grid grid-cols-1 lg:grid-cols-2 gap-8 items-center'>
                            ...
                            <div className='relative h-[300px] md:h-[400px] rounded-2xl overflow-hidden'>
                                <Image
                                    ...
                                    alt={locale === 'ar'
                                        ? 'استشارة العناية بالأسنان في عيادة توث ميت'
