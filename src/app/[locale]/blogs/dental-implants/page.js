...
export async function generateMetadata({ params }) {
    ...
    const t = await getTranslations({ locale, namespace: 'blog-implants' });

    const baseUrl = 'https://toothmatedental.com';
    const articlePath = locale === 'ar' ? '/تكلفة-زراعة-الأسنان-في-مصر/مقالاتنا' : '/our-blogs/dental-implant-cost-egypt';

    return {
        title: t('title'),
        description: t('intro'),
        keywords: "تكلفة زراعة الأسنان في مصر, أسعار زراعة الأسنان, زراعة الضرس, زرعات الأسنان, زراعة الفك الكامل, زراعة الأسنان الفورية, dental implant cost Egypt, implant prices Cairo, tooth implant, dental surgery, cosmetic dentistry",
        alternates: {
        ...
        openGraph: {
            ...
            authors: ['استشاري زراعة الأسنان'],
            url: `${baseUrl}/${locale}${articlePath}`,
            siteName: 'Tooth Mate Dental Clinic',
            images: [
                {
                    url: `${baseUrl}/images/dental-implant-hero.webp`,
                    width: 1200,
                    height: 630,
                    alt: locale === 'ar' ? 'تكلفة زراعة الأسنان في مصر - دليل شامل' : 'Dental Implant Cost in Egypt - Comprehensive Guide'
                }
            ],
            tags: ["زراعة الأسنان", "تكلفة العلاج", "طب الأسنان التجميلي"]
        },
        twitter: {
            card: 'summary_large_image',
            title: t('title'),
            description: t('intro'),
            images: [`${baseUrl}/images/dental-implant-hero.webp`],
        },
        robots: {
            index: true,
            follow: true,
        ...
        authors: [{ name: 'استشاري زراعة الأسنان' }],
...
export default async function DentalImplantsBlog({ params }) {
    const { locale } = await params;
    const t = await getTranslations({ locale, namespace: 'blog-implants' });
    ...
    const structuredData = {
        ...
        "headline": t('title'),
        "description": t('intro'),
        "image": `${baseUrl}/images/dental-implant-hero.webp`,
        "datePublished": new Date().toISOString(),
        "dateModified": new Date().toISOString(),
        "author": {
            "@type": "Person",
            "name": "استشاري زراعة الأسنان",
            "jobTitle": "استشاري زراعة الأسنان",
        },
        "publisher": {
            "@type": "Organization",
        ...
        "keywords": "تكلفة زراعة الأسنان في مصر, أسعار زراعة الأسنان, زراعة الضرس",
        "articleSection": "زراعة الأسنان",
    ...
    return (
        <scetion className='bg-[#D3F3FF]'>
            ...
            <section
                className="relative
                    bg-[url('/images/implant-photo.jpg')] 
                    bg-cover bg-center 
                    h-[400px] w-full 
                    flex justify-center 
                    items-center
                    text-white"
                aria-label={locale === 'ar' ? 'تكلفة زراعة الأسنان في مصر - دليل شامل' : 'Dental Implant Cost in Egypt - Comprehensive Guide'}
            >
            ...
            <article className='container-custom max-w-6xl mx-auto' itemScope itemType="https://schema.org/Article">
                ...
                <div className='w-full h-[350px] md:h-[450px] lg:h-[500px] relative rounded-4xl overflow-hidden my-12'>
                    <Image
                        src={'/images/implant-photo.jpg'}
                        alt={locale === 'ar'
                            ? 'توضيح مراحل زراعة الأسنان والعوامل المؤثرة على التكلفة'
                            : 'Illustration of dental implant stages and factors affecting cost'
                        }
                        fill
                ...
                {/* Dynamic sections rendering with support for lists */}
                ...
                {/* CTA Section - Dental Implant Consultation */}
                    ...
                    <div className='grid grid-cols-1 lg:grid-cols-2 gap-8 items-center'>
                        ...
                        <div className='relative h-[300px] md:h-[400px] rounded-2xl overflow-hidden'>
                            <Image
                                ...
                                alt={locale === 'ar'
                                    ? 'استشارة زراعة الأسنان في عيادة توث ميت بالمعادي'
                                    : 'Dental implant consultation at Tooth Mate clinic in Maadi'
                                }
                                fill
                                className='object-cover'
                            />
