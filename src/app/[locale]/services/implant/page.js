...
// SEO Metadata for Dental Implants Page
export async function generateMetadata({ params }) {
    const { locale } = await params;
    const t = await getTranslations('implant.metadata');

    const baseUrl = 'https://toothmatedental.com';
    const canonicalPath = locale === 'ar' ? '/زراعة-الأسنان/خدماتنا' : '/our-services/dental-implants';
    ...
    return {
        ...
        openGraph: {
            ...
            images: [
                {
                    url: `${baseUrl}/images/tooth-implant.webp`,
                    width: 1200,
                    height: 630,
                    alt: locale === 'ar' ? 'زراعة الأسنان في عيادة توث ميت' : 'Dental Implants at Tooth Mate Clinic'
                },
            ],
        },
        twitter: {
            ...
            title: t('ogTitle'),
            description: t('ogDescription'),
            images: [`${baseUrl}/images/tooth-implant.webp`],
        },
        robots: {
            index: true,
            follow: true,
...
export default async function ImplantService({ params }) {
    const { locale } = await params;
    const t = await getTranslations('implant');

    const baseUrl = 'https://toothmatedental.com';
    const canonicalPath = locale === 'ar' ? '/زراعة-الأسنان/خدماتنا' : '/our-services/dental-implants';

    // Structured Data for Dental Implants Service
    const implantStructuredData = {
    ...
    const faqStructuredData = {
        ...
        "mainEntity": [
            {
                "@type": "Question",
                "name": locale === 'ar' ? "كم تدوم زراعة الأسنان؟" : "How long do dental implants last?",
                "acceptedAnswer": {
                    "@type": "Answer",
                    "text": locale === 'ar' ? "زراعة الأسنان يمكن أن تدوم مدى الحياة مع العناية المناسبة والفحوصات الدورية" : "Dental implants can last a lifetime with proper care and regular checkups"
                }
            },
            {
                "@type": "Question",
                "name": locale === 'ar' ? "هل عملية الزراعة مؤلمة؟" : "Is the implant procedure painful?",
                "acceptedAnswer": {
                    "@type": "Answer",
                    "text": locale === 'ar' ? "نستخدم التخدير الموضعي لجعل العملية مريحة تماماً، مع متابعة ما بعد الجراحة" : "We use local anesthesia to make the procedure completely comfortable, with post-operative follow-up"
                }
            },
            {
                "@type": "Question",
                "name": locale === 'ar' ? "من هو المرشح المناسب لزراعة الأسنان؟" : "Who is a suitable candidate for dental implants?",
                "acceptedAnswer": {
                    "@type": "Answer",
                    "text": locale === 'ar' ? "البالغون بأسنان مفقودة ولديهم عظام فك صحية وصحة عامة جيدة" : "Adults with missing teeth who have healthy jaw bones and good overall health"
                }
            }
        ]
    };
    ...
    return (
        <section className='bg-[#D3F3FF]'>
            ...
            <script
                ...
                dangerouslySetInnerHTML={{ __html: JSON.stringify(implantStructuredData) }}
                key="implant-service-schema"
            />
            <script
                type="application/ld+json"
            ...
            <section itemScope itemType="https://schema.org/Service">
                {/* Hero Section */}
                <div
                    className="relative bg-[url('/images/tooth-implant.webp')] bg-cover bg-center h-[400px] w-full flex justify-center items-center text-white font-bold"
                    role="banner"
                ...
                <section className="container-custom" itemScope itemProp="mainContentOfPage">
                    ...
                    <div className="w-full h-[300px] md:h-[600px] relative rounded-4xl overflow-hidden my-8">
                        <Image
                            src="/images/implant-service-image.webp"
