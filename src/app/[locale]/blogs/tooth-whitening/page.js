...
export async function generateMetadata({ params: { locale } }) {
    const t = await getTranslations({ locale, namespace: 'blog-teeth-whitening' });

    const baseUrl = 'https://toothmatedental.com';
    const articlePath = locale === 'ar' ? '/سعر-تبييض-الأسنان-في-مصر/مقالاتنا' : '/our-blogs/teeth-whitening-price-egypt';

    return {
        title: t('title'),
        description: t('intro'),
        keywords: "سعر تبييض الأسنان في مصر, تبييض الأسنان بالليزر, تبييض الأسنان بالأشعة فوق البنفسجية, التبييض المنزلي, أسعار جلسات التبييض, teeth whitening Egypt, laser teeth whitening, UV whitening, home whitening, whitening prices, cosmetic dentistry Cairo",
        alternates: {
        ...
        openGraph: {
            ...
            images: [
                {
                    url: `${baseUrl}/images/teeth-whitening-hero.webp`,
                    width: 1200,
                    height: 630,
                    alt: t('title')
                }
            ],
            tags: ["تبييض الأسنان", "طب الأسنان التجميلي", "جمال الابتسامة"]
        },
        twitter: {
            card: 'summary_large_image',
            title: t('title'),
            description: t('intro'),
            images: [`${baseUrl}/images/teeth-whitening-hero.webp`],
        },
        robots: {
            index: true,
            follow: true,
...
export default async function TeethWhiteningBlog({ params }) {
    ...
    const structuredData = {
        ...
        "headline": t('title'),
        "description": t('intro'),
        "image": `${baseUrl}/images/teeth-whitening-hero.webp`,
        ...
        "keywords": "سعر تبييض الأسنان في مصر, تبييض الأسنان بالليزر, تبييض الأسنان بالأشعة فوق البنفسجية",
        "articleSection": "تبييض الأسنان",
    ...
    return (
        <>
            ...
            <section
                className="relative
                    bg-[url('/images/blog-teeth-whitening.jpg')] 
                    bg-cover bg-center 
                    h-[400px] w-full 
                    flex justify-center 
                    items-center
                    text-white"
                aria-label={locale === 'ar' ? 'دليل شامل حول سعر تبييض الأسنان في مصر' : 'Comprehensive Guide to Teeth Whitening Prices in Egypt'}
            >
            ...
            <article className='container-custom max-w-6xl mx-auto' itemScope itemType="https://schema.org/Article">
                ...
                <div className='w-full h-[350px] md:h-[450px] lg:h-[500px] relative rounded-4xl overflow-hidden my-12'>
                    <Image
                        src={'/images/blog-teeth-whitening.jpg'}
                        alt={locale === 'ar'
                            ? 'مقارنة بين طرق تبييض الأسنان المختلفة في مصر'
                            : 'Comparison between different teeth whitening methods in Egypt'
                        }
                        fill
                            ...
                            {section.table && (
                                <div className="my-8 overflow-x-auto bg-white rounded-2xl shadow-lg p-4">
                                    <h4 className="text-[20px] font-bold mb-4 text-[#004E6A] text-center">
                                        {locale === 'ar' ? 'مقارنة بين طرق التبييض' : 'Whitening Methods Comparison'}
                                    </h4>
                ...
                {/* CTA Section - Teeth Whitening Consultation */}
                    ...
                    <div className='grid grid-cols-1 lg:grid-cols-2 gap-8 items-center'>
                        ...
                        <div className='relative h-[300px] md:h-[400px] rounded-2xl overflow-hidden'>
                            <Image
                                ...
                                alt={locale === 'ar'
                                    ? 'استشارة تبييض الأسنان في عيادة توث ميت بالمعادي'
                                    : 'Teeth whitening consultation at Tooth Mate clinic in Maadi'
                                }
                                fill
                                className='object-cover'
                            />
