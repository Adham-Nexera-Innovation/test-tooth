...
export async function generateMetadata({ params }) {
    ...
    const t = await getTranslations({ locale, namespace: 'blog-cosmetic-fillings' });

    const baseUrl = 'https://toothmatedental.com';
    const articlePath = locale === 'ar' ? '/أنواع-الحشوات-التجميلية-في-عيادات-الأسنان/مقالاتنا' : '/our-blogs/types-of-cosmetic-dental-fillings';

    return {
        title: t('title'),
        description: t('intro'),
        keywords: "الحشوات التجميلية, حشوات الكومبوزيت, حشوات الإيماكس, حشوات الأسنان التجميلية, تجميل الأسنان, علاج تسوس الأسنان, dental composite fillings, E-max fillings, cosmetic dentistry, tooth fillings, dental restoration, tooth decay treatment",
        alternates: {
        ...
        openGraph: {
            ...
            authors: ['استشاري طب الأسنان التجميلي'],
            url: `${baseUrl}/${locale}${articlePath}`,
            siteName: 'Tooth Mate Dental Clinic',
            images: [
                {
                    url: `${baseUrl}/images/cosmetic-fillings-hero.webp`,
                    width: 1200,
                    height: 630,
                    alt: locale === 'ar' ? 'أنواع الحشوات التجميلية في عيادات الأسنان' : 'Types of Cosmetic Dental Fillings'
                }
            ],
            tags: ["الحشوات التجميلية", "تجميل الأسنان", "علاج التسوس"]
        },
        twitter: {
            card: 'summary_large_image',
            title: t('title'),
            description: t('intro'),
            images: [`${baseUrl}/images/cosmetic-fillings-hero.webp`],
        },
        robots: {
            index: true,
            follow: true,
        ...
        authors: [{ name: 'استشاري طب الأسنان التجميلي' }],
...
export default async function CosmeticFillingsBlog({ params }) {
    const { locale } = await params;
    const t = await getTranslations({ locale, namespace: 'blog-cosmetic-fillings' });
    ...
    const structuredData = {
        ...
        "headline": t('title'),
        "description": t('intro'),
        "image": `${baseUrl}/images/cosmetic-fillings-hero.webp`,
        "datePublished": new Date().toISOString(),
        "dateModified": new Date().toISOString(),
        "author": {
            "@type": "Person",
            "name": "استشاري طب الأسنان التجميلي",
            "jobTitle": "استشاري طب الأسنان التجميلي",
        },
        "publisher": {
            "@type": "Organization",
        ...
        "keywords": "الحشوات التجميلية, حشوات الكومبوزيت, حشوات الإيماكس, حشوات الأسنان",
        "articleSection": "طب الأسنان التجميلي",
    ...
    return (
        <section className='bg-[#D3F3FF]'>
            ...
            <section
                className="relative
                    bg-[url('/images/new-smile.png')] 
                    bg-cover bg-center 
                    h-[400px] w-full 
                    flex justify-center 
                    items-center
                    text-white"
                aria-label={locale === 'ar' ? 'أنواع الحشوات التجميلية في عيادات الأسنان' : 'Types of Cosmetic Dental Fillings'}
            >
            ...
            <article className='container-custom max-w-6xl mx-auto' itemScope itemType="https://schema.org/Article">
                ...
                <div className='w-full h-[350px] md:h-[450px] lg:h-[500px] relative rounded-4xl overflow-hidden my-12'>
                    <Image
                        src={'/images/Cosmetic Dental Fillings.avif'}
                        alt={locale === 'ar'
                            ? 'أنواع مختلفة من الحشوات التجميلية للأسنان'
                            : 'Different types of cosmetic dental fillings'
                        }
                        fill
                ...
                {/* CTA Section - Cosmetic Fillings Consultation */}
                    ...
                    <div className='grid grid-cols-1 lg:grid-cols-2 gap-8 items-center'>
                        ...
                        <div className='relative h-[300px] md:h-[400px] rounded-2xl overflow-hidden'>
                            <Image
                                ...
                                alt={locale === 'ar'
                                    ? 'استشارة الحشوات التجميلية في عيادة توث ميت بالمعادي'
                                    : 'Cosmetic fillings consultation at Tooth Mate clinic in Maadi'
                                }
                                fill
                                className='object-cover'
                            />
