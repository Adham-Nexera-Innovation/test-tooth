...
// SEO Metadata for Dental Prosthetics Page
export async function generateMetadata({ params }) {
    const { locale } = await params;
    const t = await getTranslations('prosthetics.metadata');

    const baseUrl = 'https://toothmatedental.com';
    const canonicalPath = locale === 'ar' ? '/تركيبات-الاسنان/خدماتنا' : '/our-services/dental-prosthetics';
            ...
            images: [
                {
                    ...
                    width: 1200,
                    height: 630,
                    alt: locale === 'ar' ? 'التركيبات السنية في عيادة توث ميت' : 'Dental Prosthetics at Tooth Mate Clinic'
                },
            ],
        },
        twitter: {
...
export default async function ProstheticsService({ params }) {
    const { locale } = await params;
    const t = await getTranslations('prosthetics');

    const baseUrl = 'https://toothmatedental.com';
    const canonicalPath = locale === 'ar' ? '/تركيبات-الاسنان/خدماتنا' : '/our-services/dental-prosthetics';

    // Structured Data for Dental Prosthetics Service
    const prostheticsStructuredData = {
        ...
        "offers": {
            "@type": "Offer",
            "description": locale === 'ar' ? "خدمات التركيبات السنية المتكاملة" : "Comprehensive dental prosthetics services"
        }
    };
    ...
    const faqStructuredData = {
        ...
        "mainEntity": [
            {
                "@type": "Question",
                "name": locale === 'ar' ? "كم تستغرق عملية تركيب التركيبات السنية؟" : "How long does dental prosthetics procedure take?",
                "acceptedAnswer": {
                    "@type": "Answer",
                    "text": locale === 'ar' ? "تختلف المدة حسب نوع التركيبة، من جلسة واحدة إلى عدة أسابيع للتركيبات الثابتة والمتحركة" : "Duration varies by prosthesis type, from a single session to several weeks for fixed and removable prosthetics"
                }
            },
            {
                "@type": "Question",
                "name": locale === 'ar' ? "كم تدوم التركيبات السنية؟" : "How long do dental prosthetics last?",
                "acceptedAnswer": {
                    "@type": "Answer",
                    "text": locale === 'ar' ? "من 5 إلى 15 سنة حسب نوع التركيبة والعناية بها" : "From 5 to 15 years depending on the prosthesis type and care"
                }
            },
            {
                "@type": "Question",
                "name": locale === 'ar' ? "هل التركيبات السنية مؤلمة؟" : "Are dental prosthetics painful?",
                "acceptedAnswer": {
                    "@type": "Answer",
    ...
    return (
        <section className='bg-[#D3F3FF]'>
            ...
            <script
                ...
                dangerouslySetInnerHTML={{ __html: JSON.stringify(prostheticsStructuredData) }}
                key="prosthetics-service-schema"
            />
            <script
                type="application/ld+json"
            ...
            <section itemScope itemType="https://schema.org/Service">
                ...
                <section className="container-custom" itemScope itemProp="mainContentOfPage">
                    ...
                    <div className="w-full h-[300px] md:h-[600px] relative rounded-4xl overflow-hidden my-8">
                        <Image
                            src="/images/prosthetics-main.webp"
                            ...
                            {[1, 2, 3, 4].map((item) => (
                                <li key={item} className="flex items-start gap-4">
                                    ...
                                    <p className="text-[18px] md:text-2xl leading-relaxed" itemProp="description">
                                        {t(`whatAreProsthetics.point${item}`)}
                                    </p>
                                </li>
                            ))}
                        </ul>
                    ...
                    {/* Types of Dental Prosthetics Section */}
                    <div className="my-12">
                        <h2 className="text-2xl md:text-3xl font-bold mb-6">
                            {t('types.title')}
                        </h2>
                        <div className="grid md:grid-cols-2 gap-8">
                            ...
                            <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-200" itemScope itemType="https://schema.org/Service">
                                <div className="flex items-center gap-4 mb-4">
                                    <Image
                                        src="/icons/dental-bridge.png"
                                        width={50}
                                        height={50}
                                        alt={t('types.bridges.iconAlt')}
                                        itemProp="image"
                                    />
                                    <h3 className="text-xl font-bold text-[#0178A3]" itemProp="name">
                                        {t('types.bridges.title')}
                                    </h3>
                                </div>
                                <p className="text-gray-700 mb-4" itemProp="description">
                                    {t('types.bridges.description')}
                                </p>
                                <div className="space-y-2">
                                    {[1, 2, 3].map((item) => (
                                        <div key={item} className="flex items-center gap-2">
                                            <div className="w-2 h-2 bg-[#0178A3] rounded-full"></div>
                                            <span className="text-sm">{t(`types.bridges.feature${item}`)}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-200" itemScope itemType="https://schema.org/Service">
                                <div className="flex items-center gap-4 mb-4">
                                    <Image
                                        src="/icons/dentures.png"
                                        width={50}
                                        height={50}
                                        alt={t('types.dentures.iconAlt')}
                                        itemProp="image"
                                    />
                                    <h3 className="text-xl font-bold text-[#0178A3]" itemProp="name">
                                        {t('types.dentures.title')}
                                    </h3>
                                </div>
                                <p className="text-gray-700 mb-4" itemProp="description">
                                    {t('types.dentures.description')}
                                </p>
                                <div className="space-y-2">
                                    {[1, 2, 3].map((item) => (
                                        <div key={item} className="flex items-center gap-2">
                                            <div className="w-2 h-2 bg-[#0178A3] rounded-full"></div>
                                            <span className="text-sm">{t(`types.dentures.feature${item}`)}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    {/* Why Choose Dental Prosthetics Banner */}
                    <div className="relative bg-[url('/images/prosthetics-main.webp')] bg-cover bg-center py-11 lg:py-14 w-full text-white font-bold rounded-4xl my-10">
                    ...
                    <div className="flex flex-col lg:flex-row justify-between items-center md:container-custom my-10">
                        ...
                        <div className={`flex justify-center items-center gap-3 order-1 lg:order-2 ${locale === 'en' ? 'lg:mr-9' : 'lg:ml-9'}`}>
                            <Image
                                src="/images/implant-toothmate.webp"
                                width={137}
                                height={112}
